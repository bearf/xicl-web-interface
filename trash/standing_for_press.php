...
require_once('./config/require.php');
  ...
  if (!$error) {
    ...
    if ($kind==1)
      $q = 'SELECT
            ...
            GROUP BY
              M1.UserId
            ORDER BY
              Solved DESC, LastAC ASC, M1.UserId ASC';
    elseif ($kind==2)
      $q = 'SELECT
              U.ID,
              U.Nickname,
    ...
    //сложный вопрос, потребует извлечения индексов задач              
      ...
      $q = 'select V.problemId, T.name from volume V inner join task T on V.taskId=T.taskId where contestid='.$contest.' order by problemId';
      ...
      if (mysql_error() != '') {
        ...
        //normal user mode
          ...
          $q_select_inner = 'SELECT '.
            ...
            'SUM(IFNULL(M.RealPts, 0)) AS `Points` ';
          ...
          $q = $q_select_main.
            'from '.
              ...
              ') S on U.id = S.userid '.
            'ORDER BY '.
              'S.Solved DESC, S.Penalty ASC, S.Points DESC, Nickname ASC';      
  ...
  $teams = array();
  $q = 'select u.id as `id`, u.nickname as `nickname` from user u';
  if (!$error) {
    $user_rec = mysql_query($q);
    if (mysql_error() != '') {
      $msg = 'Не удалось произвести запрос данных. Повторите попытку позже.';
      $error = true;
    }
    else 
      for ($i=0;$i<mysql_num_rows($user_rec);$i++) {  
        $f = mysql_fetch_array($user_rec);
        $teams[$f['id']] = $f['nickname'];
      }
  }
