...
KIR.core = function() {
    ...
    var
        ...
        ,_convert = function(element) {
            ...
            if ('string' == typeof element && 1 == jQuery('[name$=' + element.replace(/:/g, "\\:") + ']').size()) { return jQuery('[name$=' + element.replace(/:/g, "\\:") + ']').get(0); }
            // id without prefix
            if ('string' == typeof element && 1 == jQuery('[id$=' + element.replace(/:/g, "\\:") + ']').size()) { return jQuery('[id$=' + element.replace(/:/g, "\\:") + ']').get(0); }
        ...
        ,_namespace = function(element) {
            ...
            // convert
            element = _convert(element);
            if (!element) { return null; }
            // check for HTML DOM, then create and return namespace
            if (!KIR.DOM.is_DOM(element)) { throw(MESSAGE_NAMESPACE_EXPECTS_DOM_ELEMENT); }
            if (!element[KIR_NAMESPACE_NAME]) { 
                element[KIR_NAMESPACE_NAME] = {};
                // flag that it's KIR namespace
                element[KIR_NAMESPACE_NAME].namespace = element[KIR_NAMESPACE_NAME];
                // what is 'owner'?
                ...
                // *** 
                // as long as we have no "owner" object (for example
                // input I is not inside any other object)
                // then 'owner' points to I.N1 itself
                element[KIR_NAMESPACE_NAME].owner = element[KIR_NAMESPACE_NAME];
                element[KIR_NAMESPACE_NAME].html = element;
            }
    ...
    var _self = {
        ...
        ,namespace: function(element) {
            return KIR.utils.map(
                element,
                function(element) { return _namespace(element); }
            );
        } // end of namespace
