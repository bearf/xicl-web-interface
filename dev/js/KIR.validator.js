...
// --------------------------------
// todo:    refactor core, utils, convert, dom
// todo:    KIRnamespace.control -> KIRnamespace.methods
// todo:    use KIR.core.namespace everywhere if possible
// todo:    validator and events work with namespace objects
// --------------------------------
// todo:    show messages after rerender
// todo:    when form is rerendered partially it's strong needed to update messages
// todo:    add comments on this issue: no validation if validator has been just attached. Run it yourself.
// --------------------------------
...
// todo:    memory leaks? http://blog.j15r.com/2005/01/dhtml-leaks-like-sieve.html
// --------------------------------
...
KIR.validator = function() {
                            ...
                            if (name && 'target' != name) {
                                ...
                                if (validator[name] && 'function' == typeof validator[name]) {  
                                    validator[name].apply(
                                        ...
                                        KIR.utils.array(KIR.core.evaluate(attributes[name].split(';'))) // converted args
                                    );
                                } else {
    ...
    var
        ...
        ,is_control = function(html_dom_element) {
            // check for null too
            return KIR.DOM.is_DOM(html_dom_element) && KIR(html_dom_element) && KIR(html_dom_element).control;
        } // end of is_control
        ...
        ,create = function(element) {
            ...
            if (is_form(element)) {
                ...
                // KIR namespace
                _self.namespace = KIR(element);
                
                // html
                _self.html = _self.namespace.html;
                ...
                _self.validate = function() {
                    ...
                    for (var i=0; i<controls.length; i++) {
                        var control = KIR(controls[i]).validator;
                ...
                _self.controls = function() {
                    return KIR.core.filter(KIR.controls.find(f_form_html_element), 'validator');
                }; // end of "controls()"
                ...
                _self.is_valid = function() {
                    ...
                    var controls = KIR.core.filter(KIR.controls.find(f_form_html_element), 'validator');
                    for (var i=0; i<controls.length; i++) {
                        result = result && KIR(controls[i]).validator.is_valid();
                    }
                    return result;
                }; // end of is_valid
                ...
                _self.messages = function(place) {
                    ...
                    var controls = KIR.core.filter(KIR.controls.find(f_form_html_element), 'validator');
                    for (var i=0; i<controls.length; i++) {
                        KIR(controls[i]).validator.messages(place);
                        // if only one input should be shown and messages were added - exit
                        if (f_form_only_first && !KIR(controls[i]).validator.is_valid()) { break; }
                    }
                    // jQuery-style return
                    return _self;
                ...
                KIR.events(f_form_html_element).a4jsubmit(_self.check_submit);
            } // end of form methods
            ...
            if (is_control(element)) { 
                ...
                var
                    ...
                    ,   f_control = KIR(element).control      // KIR.control namespace  todo: remove
                    ,   f_control_validators = new KIR.utils.Stack()      // array of validators
                    ,   f_control_force = false                     // force form validation
                    ,   f_control_form = KIR(find_form(element)).validator    // parent form todo: simplify
                ...
                var
                    ...
                    // replace marker for param
                    ,format = function(message, params) {
                        if (params && 'object' == typeof params) {
                            for (var key in params) {
                                if (key) { message = message.replace('%' + key, params[key]); }
                            }
                        } else {
                            for (var i=0; params && i<params.length; i++) {
                                while (-1 < message.indexOf('%' + i)) { message = message.replace('%' + i, params[i]); }
                            }
                        }
                        return message;
                    } // end of format
                ...
                _self.messages = function(place) {
                    ...
                    for (var i=0; i<f_control_validators.size(); i++) {
                        if (!f_control_validators.get(i).is_valid()) {
                            ...
                            jQuery('<span id="' + f_control_form.hash() + '-' + f_control_hash + '-' + i + '" class="validation-messages">'
                                + format(f_control_validators.get(i).get_message(), [f_field])
                            + '</span>')
                                    .data('validator', _self.namespace.validator)
                                    .appendTo(place);
                ...
                _self.validate = function() {
                    ...
                    if (_self.is_valid()) {
                        jQuery(_self.html).removeClass(f_settings.invalid_class);
                        jQuery(_self.html).addClass(f_settings.valid_class);
                        f_control.valid();
                        ...
                        jQuery(_self.html).addClass(f_settings.invalid_class);
                        jQuery(_self.html).removeClass(f_settings.valid_class);
                        f_control.invalid();
                ...
                _self.attach = function(validator, params, message) {
                    ...
                    f_control_validators.push(function() {
                        ...
                        return {
                            ...
                            // replace substitutes in text with validation params
                            ,get_message: function() {
                                var message = f_message;
                                if (f_params && 'object' == typeof f_params) {
                                    var params = {}; var index = 1;
                                    for (var key in f_params) {
                                        params[index++] = f_params[key];
                                    }
                                    message = format(message, params);
                                } else if (f_params) {
                                    message = format(message, {'1' : f_params});
                                }
                                return message;
                            } // end of get_message
                ...
                _self.get_messages = function() {
                    var result = [];
                    for (var i=0; i<f_control_validators.size(); i++) {
                        if (!f_control_validators.get(i).is_valid()) { result.push(format(f_control_validators.get(i).get_message(), [f_field])); }
                    }
                    return result;
                }; // end of get_messages
                ...
                _self.equals = function(message, source) {
                    ...
                    // attach
                    // todo: check for KIR interface
                    _self.attach(_self.equals_validator, {'source' : KIR(source).control}, message);
                    // jQuery-style return
                    return _self;
                }; // end of equals
                ...
                _self.typing = function(attach) {
                    KIR.events(_self.html).typing(invoke, attach);
                    // jQuery-style return
                    return _self;
                }; // end of typing
                ...
                _self.change = function(attach) {
                    KIR.events(_self.html).change(invoke, attach);
                    // jQuery-style return
                    return _self;
                }; // end of change
                ...
                _self.blur = function(attach) {
                    KIR.events(_self.html).blur(invoke, attach);
                    // jQuery-style return
                    return _self;
                }; // end of blur
    ...
    return function(element) {
        ...
        if (!is_form(element) && !KIR(find_form(element)).validator) { KIR(find_form(element)).validator = create(find_form(element)); }
        // if no form/control object exists we create it
        if (!KIR(element).validator) { KIR(element).validator = create(element); }
        // and return
        return KIR(element).validator;
...
// end of KIR.validator object definition
    ...
    KIR.events(document).a4jcomplete(function(e, element, updated) {
        ...
        if (window.attachCustomValidators) { window.attachCustomValidators(); }
